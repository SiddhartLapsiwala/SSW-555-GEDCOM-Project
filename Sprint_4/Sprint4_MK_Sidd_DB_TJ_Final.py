...
class Repository:
    ...
    def validate_siblings_should_not_marry(self):
        ...
        for key, family in self.family.items():
            ...
            if hubby_id != "" and wifey_id != "":

                if [i for i in set(sibiling_list) if i in set(marriage_list)]:
    ...
    def validate_unique_first_names_and_birthdays(self):
        ...
        for key, family in self.family.items():
            list_of_children = list(family.children)
            count = Counter(
                [self.individual[child].name + " " + self.individual[child].birthday for child in list_of_children])
            for duplicated, value in count.items():
                if value > 1:
                    result = False
                    print(
                        "Error: US25: " + "More than one child with same name and birthday are present in family " +
                        family.id)
                    return result
        if result:
            print("US25: No families in the given file have more than one child with same name and birthday")
        ...
        for key, family in self.family.items():
            if family.divorced == 'NA':
                ...
                if today_date < marriage_date < today_date_plus30:
                    list_result.append("Husband ID: " + list(family.husband_id)[
                        0] + " Husband Name: " + family.husband_name + " Wife ID: " + list(family.wife_id)[
                                           0] + " Wife Name: " + family.wife_name)
                    result = True;
        ...
        for indi in self.individual.values():
            try:
                ...
                if today >= bday >= date:
                    result = True
                    list_result.append(
                        "Individual ID: " + indi.id + " Name: " + indi.name + " Birthday: " + indi.birthday)
            except ValueError:
                pass
    ...
    def sort_children_in_family_based_on_birthday(self):
        """US 28: List siblings in a family in sorting order"""
        sorted_result = []
        for key, family in self.family.items():
            result = []
            list_of_children = list(family.children)
            for child in list_of_children:
                result.append(self.individual[child])
            result.sort(key=lambda x: self.individual[child].age, reverse=False)

            if len(result) > 0:
                print("US 28: List siblings in a family:" + family.id + " after sorting is ")
                sorted_result.append("US 28: List siblings in a family:" + family.id + " after sorting is ")
                for child in result:
                    print(child.name + " " + str(child.age))
                    sorted_result.append(child.name + " " + str(child.age))
        return sorted_result

    def list_all_multiple_births(self):
        """US32 - List all multiple births in GEDCOM file"""
        result = []
        for key, family in self.family.items():
            birthday_list = list()
            list_of_children = family.children
            for child in list_of_children:
                birthday_list.append(self.individual[child].birthday)
            birthday = [k for k, v in Counter(birthday_list).items() if v > 1]
            for child in list_of_children:
                if len(birthday) > 0:
                    if self.individual[child].birthday == str(birthday[0]):
                        result.append("US 32:  Family Id: " + family.id + ", Child id: " + child)
                        print("US 32: Multiple birth in Family Id: " + family.id + ", Child id: " + child)
...
def main():
    ...
    """US28"""
    repo.sort_children_in_family_based_on_birthday()
    """US32"""
    repo.list_all_multiple_births()
...
class Test(unittest.TestCase):
    ...
    def test_sort_children_in_family_based_on_birthday(self):
        """US 28: List siblings in a family in descending order"""
        path = 'proj03test.ged'
        repo = Repository()
        repo.read_file(path)
        self.assertTrue(len(repo.sort_children_in_family_based_on_birthday()) > 0)
