                ...
                for i in listofchildren:
                    if i in wfe_id:
                        if hubbyid not in listofchildren:
                            print("Error: FAMILY: US17 : Husband:", hubbyid, "parent should not marry their descendants", i)
                            result = True
            for hsb_id in husband_id:
                for i in listofchildren:
                    if i in hsb_id:
                        if wifeyid not in listofchildren:
                            print("Error: FAMILY: US17 : Wife:", wifeyid, "parent should not marry their deccendants", i)
                            result = True
        return result

    def validate_sibilings_should_not_marry(self):
        ...
        for key, family in self.family.items():
            ...
            if set(marriage_list) == set(sibiling_list):
                print("Error: FAMILY: US18 : Brother:", hubby_id, "sibiling should not marry Sister:", wifey_id)
                result = True
...
def main():
    ...
    """US18"""
    repo.validate_sibilings_should_not_marry()
    """US21"""
...
class Test(unittest.TestCase):
    ...
    def test_validate_sibilings_should_not_marry(self):
        ...
        repo = Repository()
        repo.read_file(path)
        self.assertEqual(repo.validate_sibilings_should_not_marry(), True)
        self.assertNotEqual(repo.validate_sibilings_should_not_marry(), False)
        self.assertTrue(repo.validate_sibilings_should_not_marry())
        self.assertIsNotNone(repo.validate_sibilings_should_not_marry())
        self.assertIsNot(repo.validate_sibilings_should_not_marry(), '')
